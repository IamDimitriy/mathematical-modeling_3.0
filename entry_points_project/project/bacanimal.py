...
class Bacteria(pygame.sprite.Sprite):
    ...
    def __init__(self, resist, immunity, crispr_cas_st, crispr, img, x, y, sensitive=default_sensitive):
        ...
        self.last_move = 0
        self.sensitive = sensitive  # чутьё
        # структура "ближайший сосед"
        self.neighbour = [[BOX_WIGHT * BOX_WIGHT + BOX_HEIGHT * BOX_HEIGHT, BOX_WIGHT * BOX_WIGHT + BOX_HEIGHT * BOX_HEIGHT],
                          [-1, -1],
                          [-1, -1]]
        # первый столбик здесь - ближайшая бактерия. Второй - ближайшая еда
        # первая строка - расстояние до цели, вторая - её Х координата, третья - её Y координата
        # если появится разделение на больших и малых бактерий, добавить третий столбик

    def infection(self, x, y):
        global quantity
        ingress = fac.ingress  # здесь для всех заражений берутся значения только одного фага
        suppression = fac.suppression
        hits = pygame.sprite.groupcollide(norm_bacs, red_facs, False, False)
        if hits:
            ...
            if penetration >= 0:
                ...
                if not fac.color in self.crispr:
                    if infection >= 0:
                        # кол-во, фагов новых
                        quantity = int(abs(1-infection * 25))
    ...
    def locator(self):  # функция, определяющая, есть ли бактерии в круге чутья
    ...
    def update(self):
        """
        if self in crispr_bacs:
            print(self.immunity, self.resist, "CrISPr")
        elif self in non_crispr:
            print(self.immunity, self.resist, "NO")
        """
        choice_list = ['self.change_x', 'self.change_y', 'none']
        name = random.choice(choice_list)  # выбор стороны движения
        self.old_eng = self.eng
        if self in norm_bacs:  # питаются только здоровые
...
class Bacteriophage(pygame.sprite.Sprite):
    def __init__(self, color, x, y):
        mutation = random.uniform(-0.05, 0.05)
        self.ingress = random.uniform(0.3, 1.1) + mutation
        self.suppression = random.uniform(0.3, 1.15) + mutation
        print(self.ingress, self.suppression, "параметры фага")
        pygame.sprite.Sprite.__init__(self)
        self.color = color
        if self.color == RED:
